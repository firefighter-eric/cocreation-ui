...
- 支持正式实验入口，可选择 `和AI开始` 或 `和人开始`，连续完成全部 6 个 prompt
- 左侧栏用于切换模式、选择开场句、重新开始
- 右侧以聊天式故事流展示当前共创过程
- 右上角支持打开设置抽屉，修改完整 `system prompt`、创作风格、`model`、`temperature`、`top_p` 和与人对话延迟倍率
...
- `正式实验`：点击左侧“开始实验”后，选择 `和AI开始` 或 `和人开始`，系统会按随机顺序推进 6 个 prompt，并把起手方严格平衡为 3:3；连续多轮实验时会尽量避免首题重复
